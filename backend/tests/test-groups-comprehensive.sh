...
GROUP_ID=$(echo "$RESPONSE" | grep -o '"_id":"[^"]*' | head -1 | cut -d'"' -f4)
INVITE_CODE=$(echo "$RESPONSE" | grep -o '"inviteCode":"[^"]*' | cut -d'"' -f4)
...
if echo "$RESPONSE" | grep -q '"_id"'; then
    print_success "Grupos listados correctamente"
else
...
if echo "$RESPONSE" | grep -q '"name":"Grupo de Prueba"'; then
...
if echo "$RESPONSE" | grep -q '"name"'; then
    print_success "Miembros listados correctamente"
else
    print_error "Error al listar miembros"
fi
...
if echo "$RESPONSE" | grep -q '"name":"Grupo Actualizado"'; then
...
NEW_INVITE_CODE=$(echo "$RESPONSE" | grep -o '"inviteCode":"[^"]*' | cut -d'"' -f4)
...
# Si tiene "success":false, significa que la expulsión fue bloqueada correctamente
if echo "$RESPONSE" | grep -q '"success":false'; then
    print_success "Expulsión bloqueada correctamente (sin permisos)"
else
    if echo "$RESPONSE" | grep -q '"error"'; then
        print_success "Expulsión bloqueada (error de permisos)"
    fi
fi
...
if echo "$RESPONSE" | grep -q '"_id"'; then
    print_success "Miembros verificados"
else
    print_error "Error: no se pudieron obtener los miembros"
fi
...
# Si tiene error, significa que la salida fue bloqueada correctamente
if echo "$RESPONSE" | grep -q '"error"'; then
    print_success "Salida bloqueada correctamente (es admin)"
else
    if echo "$RESPONSE" | grep -q '"success":false'; then
        print_success "Salida bloqueada (admin no puede salir)"
    fi
fi
...
print_success "Grupos listados después de eliminación"
